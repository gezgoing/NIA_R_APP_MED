...
## Target diagnosis code: I10 idx = (table_20$MSICK_CD_head=='I10' |
table_20$SSICK_CD_head=='I10')
...
unused = c('key', 'no', 'LN_NO', 'MSICK_CD', 'SSICK_CD', 'RECU_FR_DD', 'RECU_TO_DD', 'RECU_DDCNT', 'VST_DDCNT', 'yno', 'DIV_CD', 'GNL_NM_CD')
...
xgb = xgboost(data=as.matrix(x), label=as.matrix(y),
				...
				verbose=TRUE,
				print.every.n=10
				# early.stop.round=100 # stop if no improvement
within 100 trees
)